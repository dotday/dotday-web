...
import type { GroundConditionCardsData } from "@/lib/home/types";

/**
 * UseCases / groundConditionCards - "Built for every ground condition." Three
 * cards mapping a real ground condition to the correct DOTDAY fabric. Honest
 * product fit is a brand rule: gravel -> XBAR, beds -> SHIELD, drainage -> TERRA.
 *
 * Now data-driven; markup is byte-identical (same ucase-* / ucard classes), so
 * the homepage renders pixel-for-pixel the same. DEFAULT_GROUND_CONDITION_CARDS
 * holds today's exact content for the Option-A migration.
 */

export const DEFAULT_GROUND_CONDITION_CARDS: GroundConditionCardsData = {
  _type: "groundConditionCards",
  badge: "Built for every ground condition",
  heading: "Match the fabric to the ground, not the price tag.",
  intro:
    "The wrong weight fails months later as weed pressure, shifting gravel, or drainage problems. Here is the right call for each job.",
  cards: [
    {
    ...
    },
    {
      ...
      image: "/products/shield-card.webp",
    ...
    },
    {
      ...
      image: "/products/terra-card.webp",
...
export function UseCases({
  data = DEFAULT_GROUND_CONDITION_CARDS,
}: {
  data?: GroundConditionCardsData;
}) {
  return (
